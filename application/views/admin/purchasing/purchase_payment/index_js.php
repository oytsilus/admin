		  ...
		  buttons: [
			{
              ...
              action: function ( e, dt, node, config ) {
                location.href="<?php echo base_url('purchase_payment/create');?>";
              },
		...
		if(!$(this).hasClass('disablee')) {
			bootbox.confirm("Hapus data ini?", function(result) {                
				if (result === true) {                                               
					$.ajax({
						type: 'POST',
						url: base_url+'purchase_payment/delete',
						data: {id:id,tp_id:tp_id},
						dataType: 'json',
						success: function(data) {
								parent.fadeOut(1000,function() {
									parent.remove();
									
									setTimeout(function(){ window.location=base_url+"purchase_payment"; }, 500);
								});                    
						}
					});                         
				}
