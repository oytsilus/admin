			...
			<table id="datatable-buttons" class="table table-striped table-bordered">
			  ...
			  <tbody>
				...
				foreach($result as $row) {
					$nextId = $this->no_format->get_next_pp($row['tpi_id'],$row['tp_id']);
					...
					$url = ($nextId > 0 ? 'javascript:;' : base_url('purchase_payment/edit/'.$row['tpi_id']));
					//$disabled = NULL;
					//$url = base_url('purchase_payment/edit/'.$row['tpi_id']);
				?>
				<tr>
				  <td>
					<a class="detail-record" href="<?php echo base_url('purchase_payment/print_payment/'.$row['tpi_id']);?>" title="Detail" target="_blank"><?php echo $row['tpi_no'];?></a>
				  </td>
				  <td><?php echo $this->tanggal->tgl_postgres($row['tpi_date']);?></td>
				  <td><a class="detail-record" href="<?php echo base_url('purchase_order/print_po/'.$row['tp_id']);?>" title="Detail PO" target="_blank"><?php echo $row['tp_no'];?></a></td>
				  ...
				  <td class="text-center">
					...
					<a href="javascript:;" class="btn btn-danger btn-xs btn_delete<?php echo $class;?>" data-id="<?php echo $row['tpi_id'];?>" data-tp-id="<?php echo $row['tp_id'];?>"<?php echo $disabled.$title;?>><i class="fa fa-trash-o"></i> Delete </a>
				  </td>
				</tr>
				<?php
				$no++;
